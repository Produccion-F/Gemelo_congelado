...
:root {
    ...
    --light-gray: #f9f9f9;    /* Un gris muy suave */
...
.tunnel-grid-container { display: grid; grid-template-columns: repeat(var(--cols, 10), 1fr); gap: 3px; border: 2px solid var(--dark-blue); background-color: var(--light-gray); padding: 5px; border-radius: 5px; max-width: 300px; margin-left: auto; margin-right: auto; }
...
class Tunnel:
    ...
    def get_html_viz(self):
        num_pallets_llenos = min(int(round(self.pallets_actual)), self.cols * self.rows); total_celdas = self.cols * self.rows; celdas_html = "".join([f'<div class="tunnel-cell {"filled" if i < num_pallets_llenos else ""}"></div>' for i in range(total_celdas)]); pallets_str = f"{num_pallets_llenos} / {int(self.max_pallets)}"; kg_str = f"{self.kg_actual:,.0f}".replace(',', '.') + f" / {self.max_kg:,.0f}".replace(',', '.')
        # Asegurarse que el HTML se genera correctamente
        return f"""<div class="tunnel-visualization"><div class="tunnel-label">{self.name}</div><div class="tunnel-stats">{pallets_str} palés<br>({kg_str} kg)</div><div class="tunnel-grid-container" style="--cols: {self.cols};">{celdas_html}</div></div>"""

# --- Título ---
st.markdown("<h1 style='text-align: center;'>🏭 Gemelo Digital Congelación V.1", unsafe_allow_html=True)
...
with tab_cfg:
    ...
    with col_g1: st.number_input("Horas Totales (Base)", value=st.session_state.duracion_simulacion, min_value=1, key="duracion_simulacion")
    with col_g2: st.number_input("Kg Iniciales Cámara", value=st.session_state.kg_iniciales_camara, min_value=0, key="kg_iniciales_camara")
    with col_g3: st.date_input("Fecha Inicio Simulación", value=st.session_state.fecha_inicio, key="fecha_inicio", format="DD.MM.YYYY")
    st.markdown("<h4>Inventario Inicial en Túneles</h4>", unsafe_allow_html=True)
    col_t1, col_t2, col_t3 = st.columns(3)
    with col_t1: st.number_input("Kg Iniciales YA Congelados (Total)", value=st.session_state.kg_iniciales_tunel_congelado, min_value=0, key="kg_iniciales_tunel_congelado", help="Listos para sacar.")
    with col_t2: st.number_input("Kg Iniciales AÚN Congelando (Total)", value=st.session_state.kg_iniciales_tunel_frescos, min_value=0, key="kg_iniciales_tunel_frescos", help="Deben esperar.")
    if st.session_state.kg_iniciales_tunel_frescos > 0:
        with col_t3: st.number_input("Horas Restantes Congelación", value=st.session_state.horas_restantes_congelacion, min_value=1, max_value=23, key="horas_restantes_congelacion", help="¿Cuántas horas les falta?")
    ...
    st.number_input("Hora Inicio Despiece (0-23)", value=st.session_state.d_inicio, min_value=0, max_value=23, key="d_inicio"); st.number_input("Cerdos/DÍA", value=st.session_state.d_cerdos, key="d_cerdos"); st.number_input("Velocidad Teórica (cerdos/h)", value=st.session_state.d_velo, key="d_velo"); st.slider("OEE (%)", 1, 100, value=st.session_state.d_oee, key="d_oee"); st.number_input("Peso Medio Canal (kg)", value=st.session_state.d_peso, key="d_peso"); st.checkbox("¿Día extra?", key="d_extra_check")
    if st.session_state.d_extra_check:
        with st.expander("Parámetros Día Extra (Despiece)"): st.number_input("Hora Inicio Extra", value=st.session_state.d_inicio_extra, min_value=0, max_value=23, key="d_inicio_extra"); st.number_input("Cerdos Extra", value=st.session_state.d_cerdos_extra, key="d_cerdos_extra")

    # --- Salidas Cámara ---
    st.markdown("---"); st.markdown("<h2>🧊 Salidas Cámara (Diario)</h2>", unsafe_allow_html=True)
    with st.expander("📦 Cajas (Alimenta Túneles)"):
        st.number_input("Hora Inicio Cajas (0-23)", value=st.session_state.c_inicio, min_value=0, max_value=23, key="c_inicio"); st.number_input("Horas Trabajo Cajas", value=st.session_state.c_duracion, min_value=1, key="c_duracion")
        lineas_cajas = [];
        for i in range(7): default_line_val = st.session_state.get(f"c_linea_{i}", 1250); lineas_cajas.append(st.number_input(f"Línea {i+1} (kg/h)", value=default_line_val, min_value=0, key=f"c_linea_{i}"))
        ...
        if st.session_state.c_extra_check:
            with st.expander("Parámetros Día Extra (Cajas)"): st.number_input("Hora Inicio Extra", value=st.session_state.c_inicio_extra, min_value=0, max_value=23, key="c_inicio_extra"); st.number_input("Horas Trabajo Extra", value=st.session_state.c_duracion_extra, min_value=1, key="c_duracion_extra"); st.number_input("Total Cajas (kg/h) Extra", value=st.session_state.c_kg_extra, min_value=0, key="c_kg_extra")
    with st.expander("📉 Placas"):
         st.number_input("Hora Inicio Placas (0-23)", value=st.session_state.p_inicio, min_value=0, max_value=23, key="p_inicio"); st.number_input("Horas Trabajo Placas", value=st.session_state.p_duracion, min_value=1, key="p_duracion"); st.number_input("Placas (kg/h)", value=st.session_state.p_kg, min_value=0, key="p_kg"); st.checkbox("¿Día extra?", key="p_extra_check")
         if st.session_state.p_extra_check:
             with st.expander("Parámetros Día Extra (Placas)"): st.number_input("Hora Inicio Extra", value=st.session_state.p_inicio_extra, min_value=0, max_value=23, key="p_inicio_extra"); st.number_input("Horas Trabajo Extra", value=st.session_state.p_duracion_extra, min_value=1, key="p_duracion_extra"); st.number_input("Placas (kg/h) Extra", value=st.session_state.p_kg_extra, min_value=0, key="p_kg_extra")
    with st.expander("🚛 Fresco"):
         st.number_input("Hora Inicio Fresco (0-23)", value=st.session_state.f_inicio, min_value=0, max_value=23, key="f_inicio"); st.number_input("Horas Trabajo Fresco", value=st.session_state.f_duracion, min_value=1, key="f_duracion"); st.number_input("Kg/DÍA Fresco", value=st.session_state.f_kg_dia, key="f_kg_dia"); st.checkbox("¿Día extra?", key="f_extra_check")
         if st.session_state.f_extra_check:
             with st.expander("Parámetros Día Extra (Fresco)"): st.number_input("Hora Inicio Extra", value=st.session_state.f_inicio_extra, min_value=0, max_value=23, key="f_inicio_extra"); st.number_input("Horas Trabajo Extra", value=st.session_state.f_duracion_extra, min_value=1, key="f_duracion_extra"); st.number_input("Kg Extra Fresco", value=st.session_state.f_kg_dia_extra, key="f_kg_dia_extra")

    # --- Salida Túnel ---
    st.markdown("---"); st.markdown("<h2>❄️ Salida Túnel (Diario)</h2>", unsafe_allow_html=True)
    with st.expander("📤 Vaciado (General)"):
         st.number_input("Hora Inicio Vaciado (0-23)", value=st.session_state.v_inicio, min_value=0, max_value=23, key="v_inicio"); st.number_input("Horas Trabajo Vaciado", value=st.session_state.v_duracion, min_value=1, key="v_duracion"); st.number_input("Capacidad Vaciado (kg/h)", value=st.session_state.v_kg, min_value=0, key="v_kg"); st.checkbox("¿Día extra?", key="v_extra_check")
         if st.session_state.v_extra_check:
             with st.expander("Parámetros Día Extra (Vaciado)"): st.number_input("Hora Inicio Extra", value=st.session_state.v_inicio_extra, min_value=0, max_value=23, key="v_inicio_extra"); st.number_input("Horas Trabajo Extra", value=st.session_state.v_duracion_extra, min_value=1, key="v_duracion_extra"); st.number_input("Capacidad Vaciado (kg/h) Extra", value=st.session_state.v_kg_extra, min_value=0, key="v_kg_extra")
...
with tab_sim:
    ...
    placeholder_viz = st.empty() # Placeholder para visualización RESTAURADO
    ...
    if st.button("Iniciar Simulación", key="start_sim_button", type="primary"):
        ...
        if 'v_extra_check' not in st.session_state:
            ...
            v_real = st.session_state.d_velo * (st.session_state.d_oee / 100.0); kg_por_hora_despiece = v_real * st.session_state.d_peso; kg_por_dia_despiece_total = st.session_state.d_cerdos * st.session_state.d_peso; horas_trabajo_despiece = (st.session_state.d_cerdos / v_real) if v_real > 0 else 0; kg_hora_fresco = (st.session_state.f_kg_dia / st.session_state.f_duracion) if st.session_state.f_duracion > 0 else 0; fin_despiece = st.session_state.d_inicio + horas_trabajo_despiece; fin_cajas = st.session_state.c_inicio + st.session_state.c_duracion; fin_placas = st.session_state.p_inicio + st.session_state.p_duracion; fin_fresco = st.session_state.f_inicio + st.session_state.f_duracion; fin_vaciado = st.session_state.v_inicio + st.session_state.v_duracion;
            if st.session_state.d_extra_check: kg_por_hora_despiece_extra = v_real * st.session_state.d_peso; kg_por_dia_despiece_total_extra = st.session_state.d_cerdos_extra * st.session_state.d_peso; horas_trabajo_despiece_extra = (st.session_state.d_cerdos_extra / v_real) if v_real > 0 else 0; fin_despiece_extra = st.session_state.d_inicio_extra + horas_trabajo_despiece_extra
            ...
            with placeholder_resumen.container():
                st.markdown("<h3>📊 Resumen de Flujos (Días Normales)</h3>", unsafe_allow_html=True); col1, col2, col3 = st.columns(3)
                with col1: st.markdown("<h5 style='text-align: center; color:var(--dark-blue)'>🐷 Producción</h5>", unsafe_allow_html=True); st.metric("Kg/Día", f"{kg_por_dia_despiece_total:,.0f}".replace(',', '.') + " kg"); st.metric("Ritmo", f"{kg_por_hora_despiece:,.0f}".replace(',', '.') + " kg/h"); st.metric("Horas", f"{horas_trabajo_despiece:.2f} h/día")
                with col2: st.markdown("<h5 style='text-align: center; color:var(--dark-blue)'>🧊 Salidas Cámara</h5>", unsafe_allow_html=True); st.metric("Ritmo Cajas", f"{st.session_state.kg_hora_cajas_total:,.0f}".replace(',', '.') + " kg/h"); st.metric("Ritmo Placas", f"{st.session_state.p_kg:,.0f}".replace(',', '.') + " kg/h"); st.metric("Ritmo Fresco", f"{kg_hora_fresco:,.0f}".replace(',', '.') + " kg/h")
                with col3: st.markdown("<h5 style='text-align: center; color:var(--dark-blue)'>❄️ Salida Túneles</h5>", unsafe_allow_html=True); st.metric("Capacidad Vaciado", f"{st.session_state.v_kg:,.0f}".replace(',', '.') + " kg/h")
                st.markdown("---")
            ...
            kg_iniciales_congelado = float(st.session_state.kg_iniciales_tunel_congelado)
            kg_iniciales_frescos = float(st.session_state.kg_iniciales_tunel_frescos)
            horas_restantes = int(st.session_state.horas_restantes_congelacion) if st.session_state.kg_iniciales_tunel_frescos > 0 else 0
            hora_entrada_frescos = -(24 - horas_restantes) if horas_restantes > 0 else -999
            ...
            for hora_actual in range(1, duracion_total_real + 1):
                ...
                if (es_dia_extra and st.session_state.d_extra_check):
                    if (hora_del_dia >= st.session_state.d_inicio_extra) and (hora_del_dia < fin_despiece_extra) and (kg_procesados_despiece_hoy < kg_por_dia_despiece_total_extra):
                        ...
                        kg_camara_fresco += kg_hora; kg_procesados_despiece_hoy += kg_hora
                ...
                if (es_dia_extra and st.session_state.f_extra_check):
                    if (hora_del_dia >= st.session_state.f_inicio_extra) and (hora_del_dia < fin_fresco_extra) and (kg_cargados_fresco_hoy < st.session_state.f_kg_dia_extra):
                        ...
                        procesado_fresco = min(demanda_fresco, kg_camara_fresco); kg_camara_fresco -= procesado_fresco; kg_cargados_fresco_hoy += procesado_fresco
                elif (not es_dia_extra):
                ...
                with placeholder_metricas.container():
                    ...
                    label_tunel = "❄️ Inv. Túneles" + (" ⚠️" if kg_congelar_fuera > 0 else "")
                ...
                with placeholder_grafico.container():
                    ...
                    st.area_chart(df_historial, height=400, color=["#3498db", "#e67e22", "#e74c3c"]) # Azul, Naranja, Rojo

                # RESTAURADO Y VERIFICADO: Actualizar Visualización de Túneles
                with placeholder_viz.container():
                    st.markdown("<h6>Ocupación Túneles (Palés)</h6>", unsafe_allow_html=True)
                    cols_viz = st.columns(5)
                    # Asegurarse que se renderiza incluso si tuneles está vacío (aunque no debería)
                    if tuneles:
                        for i, tunel in enumerate(tuneles):
                            # Añadir try-except por si falla la generación del HTML
                            try:
                ...
                with placeholder_progreso.container():
                    st.markdown("---"); col_p1, col_p2 = st.columns(2)
            ...
            with placeholder_tabla_final.container():
                st.markdown("---"); st.markdown("<h3>📈 Resumen Inventarios Fin de Día (Kg)</h3>", unsafe_allow_html=True)
