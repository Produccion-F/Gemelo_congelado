...
# (Clase Tunnel sin cambios, copiada de V.6)
class Tunnel:
    def __init__(self, name, max_kg, max_pallets, rows, cols):
        ...
        self.pallets_huesos = 0.0 # Contador específico para visualización
        self.pallets_carne = 0.0 # Contador específico para visualización
        self.affinity = "None" # Nuevo estado (None, Huesos, Carne, Mixed)
    ...
    def add_kg(self, kg_a_anadir, hora_actual, tipo_producto, force_mix=False):
        ...
        if (self.affinity != "None" and self.affinity != "Mixed" and 
            ...
            return kg_a_anadir 
        kg_caben_por_peso = self.get_kg_disponibles()
        pallets_caben = self.get_pallets_disponibles()
        kg_caben_por_pallets = pallets_caben * self.avg_kg_per_pallet
        kg_que_realmente_caben = max(0, min(kg_a_anadir, kg_caben_por_peso, kg_caben_por_pallets))
        kg_sobrantes = kg_a_anadir - kg_que_realmente_caben
        if kg_que_realmente_caben > 0.01:
            horas_congelacion = 18 if tipo_producto == "Huesos" else 33
            pallets_a_anadir = kg_que_realmente_caben / self.avg_kg_per_pallet
            self.queue.append({
                "kg": kg_que_realmente_caben, "pallets": pallets_a_anadir, 
                "hora_entrada": hora_actual, "tipo_producto": tipo_producto, 
                "horas_congelacion": horas_congelacion
            })
            self.kg_actual += kg_que_realmente_caben
            self.pallets_actual += pallets_a_anadir
            if tipo_producto == "Huesos": self.pallets_huesos += pallets_a_anadir
            else: self.pallets_carne += pallets_a_anadir
            self.update_affinity()
        return kg_sobrantes

    def add_initial_kg(self, kg_iniciales, hora_entrada_calculada, porcentaje_huesos):
        kg_a_meter = min(kg_iniciales, self.get_kg_disponibles())
        pallets_a_meter_max = self.get_pallets_disponibles()
        kg_caben_por_pallets = pallets_a_meter_max * self.avg_kg_per_pallet
        kg_a_meter = min(kg_a_meter, kg_caben_por_pallets)
        kg_metidos_total = 0.0
        if kg_a_meter > 0.01 and self.avg_kg_per_pallet > 0:
            if hora_entrada_calculada == -999:
                pallets_a_meter = kg_a_meter / self.avg_kg_per_pallet
                self.queue.appendleft({
                    "kg": kg_a_meter, "pallets": pallets_a_meter, 
                    "hora_entrada": -999, "tipo_producto": "Congelado", "horas_congelacion": 0
                })
                self.kg_actual += kg_a_meter; self.pallets_actual += pallets_a_meter
                kg_metidos_total = kg_a_meter
            else:
                kg_huesos = kg_a_meter * (porcentaje_huesos / 100.0)
                kg_carne = kg_a_meter - kg_huesos
                if kg_carne > 0.01: 
                    pallets_carne = kg_carne / self.avg_kg_per_pallet
                    self.queue.appendleft({
                        "kg": kg_carne, "pallets": pallets_carne, 
                        "hora_entrada": hora_entrada_calculada, "tipo_producto": "Carne", "horas_congelacion": 33
                    })
                    self.kg_actual += kg_carne; self.pallets_actual += pallets_carne
                    self.pallets_carne += pallets_carne; kg_metidos_total += kg_carne
                if kg_huesos > 0.01: 
                    pallets_huesos = kg_huesos / self.avg_kg_per_pallet
                    self.queue.appendleft({
                        "kg": kg_huesos, "pallets": pallets_huesos, 
                        "hora_entrada": hora_entrada_calculada, "tipo_producto": "Huesos", "horas_congelacion": 18
                    })
                    self.kg_actual += kg_huesos; self.pallets_actual += pallets_huesos
                    self.pallets_huesos += pallets_huesos; kg_metidos_total += kg_huesos
        self.update_affinity() 
    ...
    def vaciar_kg(self, kg_a_vaciar_disponibles, hora_actual):
        ...
        while kg_a_vaciar_disponibles > 0.01 and len(self.queue) > 0:
            ...
            if listo_para_sacar:
                ...
                pallets_a_sacar_del_lote = kg_a_sacar_del_lote / self.avg_kg_per_pallet
...
# --- Título ---
st.markdown("<h1 style='text-align: center;'>Gemelo digital congelación V.3", unsafe_allow_html=True) # <--- CAMBIO: Título
...
# (Pestaña 1 sin cambios, copiada de V.6)
with tab_cfg:
    ...
    if st.button("Recargar Configuración desde Google Sheet", key="reload_button"):
        ...
        st.session_state.pop('config_loaded', None) # Forzar recarga
        st.rerun()
...
with tab_sim:
    ...
    if st.button("Iniciar Simulación", key="start_sim_button", type="primary"):
        ...
        if 'v_extra_check' not in st.session_state:
            ...
            # --- INICIO BUCLE ---
            tuneles = [ Tunnel("CC037", 28600, 44, 11, 4), Tunnel("CC038", 28600, 44, 11, 4), Tunnel("CC058", 28600, 44, 11, 4), Tunnel("CC059", 35750, 55, 11, 5), Tunnel("CC062", 109000, 168, 21, 8)]
            ...
            # <--- CAMBIO: Inicializar el nuevo acumulador ---
            kg_total_congelados_acumulado = 0.0

            # (Distribución inicial de KG sin cambios)
            ...
            if kg_iniciales_congelado > 0:
                ...
                for i, t in enumerate(tuneles):
                    metidos = t.add_initial_kg(kg_a_distribuir_congelado, hora_entrada_calculada=-999, porcentaje_huesos=pct_huesos)
            ...
            if kg_iniciales_frescos > 0:
                ...
                for i, t in enumerate(tuneles):
                    metidos = t.add_initial_kg(kg_a_distribuir_frescos, hora_entrada_calculada=hora_entrada_frescos, porcentaje_huesos=pct_huesos)
            ...
            for hora_actual in range(1, duracion_total_real + 1):
                ...
                if kg_a_distribuir_cajas > 0: 
                    ...
                    # (Lógica de afinidad sin cambios)
                    for tunel in tuneles:
                        if kg_huesos_hora > 0.01: kg_huesos_hora = tunel.add_kg(kg_huesos_hora, hora_actual, "Huesos", force_mix=False)
                    for tunel in tuneles:
                        if kg_carne_hora > 0.01: kg_carne_hora = tunel.add_kg(kg_carne_hora, hora_actual, "Carne", force_mix=False)
                    for tunel in tuneles:
                        if kg_huesos_hora > 0.01: kg_huesos_hora = tunel.add_kg(kg_huesos_hora, hora_actual, "Huesos", force_mix=True)
                    for tunel in tuneles:
                        if kg_carne_hora > 0.01: kg_carne_hora = tunel.add_kg(kg_carne_hora, hora_actual, "Carne", force_mix=True)
                    kg_congelar_fuera += kg_huesos_hora + kg_carne_hora
                
                # B. Placas
                # <--- CAMBIO: Inicializar procesado_placas ---
                ...
                # <--- CAMBIO: Actualizar Placeholders Métricas (4 columnas) ---
